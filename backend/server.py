...
async def create_reminders_for_shipment(shipment_id, user_id, container_number, last_free_day, user_settings):
    try:
        lfd = datetime.fromisoformat(last_free_day.replace('Z', '+00:00'))
        now = datetime.now(timezone.utc)
...
def send_real_sms(to_number: str, message: str) -> dict:
    ...
    try:
        sms = twilio_client.messages.create(body=message, from_=TWILIO_PHONE_NUMBER, to=to_number)
...
async def signup(user_data: UserCreate):
    ...
    user_doc = {
        ...
        "notification_settings": {"notify_48h": True, "notify_24h": True, "notify_12h": True, "notify_6h": True},
        "created_at": datetime.now(timezone.utc).isoformat()
    }
    ...
    return {
        ...
        "user": {
            ...
            "company_name": user_data.company_name, "phone": user_data.phone,
            "forwarding_email": forwarding_email, "inbound_email": inbound_email,
...
async def login(credentials: UserLogin):
    ...
    return {
        ...
        "user": {
            ...
            "company_name": user["company_name"], "phone": user.get("phone"),
            "forwarding_email": user.get("forwarding_email"), "inbound_email": user.get("inbound_email"),
...
async def get_me(current_user: dict = Depends(get_current_user)):
    return {
        ...
        "company_name": current_user["company_name"], "phone": current_user.get("phone"),
        "forwarding_email": current_user.get("forwarding_email"), "inbound_email": current_user.get("inbound_email"),
...
async def get_shipments(current_user: dict = Depends(get_current_user)):
    result = await sb(supabase.table('shipments').select('*').eq('user_id', current_user['id']).order('last_free_day', desc=False))
    ...
    for s in shipments:
        ...
        output.append({
            ...
            "container_number": s.get("container_number", ""), "vessel_name": s.get("vessel_name", "Unknown"),
            "carrier": s.get("carrier", ""), "arrival_date": s.get("arrival_date", ""),
            "last_free_day": s.get("last_free_day", ""), "notes": s.get("notes"),
            "status": status, "hours_remaining": round(hours, 1),
            "created_at": s.get("created_at", ""), "source": s.get("source", "manual"),
            "picked_up": s.get("picked_up", False), "picked_up_at": s.get("picked_up_at"),
...
async def create_shipment(shipment_data: ShipmentCreate, current_user: dict = Depends(get_current_user)):
    ...
    shipment_doc = {
        ...
        "vessel_name": shipment_data.vessel_name, "arrival_date": shipment_data.arrival_date,
        "last_free_day": shipment_data.last_free_day, "notes": shipment_data.notes,
        "status": status, "hours_remaining": round(hours, 1),
        "created_at": datetime.now(timezone.utc).isoformat(), "source": "manual",
        "fees_avoided": 0, "picked_up": False
    }
    await sb(supabase.table('shipments').insert(shipment_doc))
    user_settings = current_user.get("notification_settings") or {}
    await create_reminders_for_shipment(shipment_id, current_user["id"], shipment_data.container_number.upper(), shipment_data.last_free_day, user_settings)
    return shipment_doc
...
async def delete_shipment(shipment_id: str, current_user: dict = Depends(get_current_user)):
    result = await sb(supabase.table('shipments').select('id').eq('id', shipment_id).eq('user_id', current_user['id']).maybe_single())
    if not result.data:
...
async def get_shipment_stats(current_user: dict = Depends(get_current_user)):
    ...
    stats = {"total": len(shipments), "safe": 0, "warning": 0, "critical": 0, "expired": 0, "picked_up": 0, "active": 0, "potential_fees_avoided": lifetime_savings}
    for s in shipments:
        if s.get("picked_up"):
            stats["picked_up"] += 1
        else:
...
async def mark_shipment_picked_up(shipment_id: str, current_user: dict = Depends(get_current_user)):
    result = await sb(supabase.table('shipments').select('*').eq('id', shipment_id).eq('user_id', current_user['id']).maybe_single())
    shipment = result.data
    if not shipment:
        raise HTTPException(status_code=404, detail="Shipment not found")
    _, hours_remaining = calculate_shipment_status(shipment.get("last_free_day", ""))
    fees_avoided = 300 if hours_remaining > 0 else 0
    await sb(supabase.table('shipments').update({"picked_up": True, "picked_up_at": datetime.now(timezone.utc).isoformat(), "fees_avoided": fees_avoided}).eq('id', shipment_id))
    user_result = await sb(supabase.table('users').select('lifetime_fees_avoided').eq('id', current_user['id']).maybe_single())
    current_savings = (user_result.data or {}).get('lifetime_fees_avoided', 0)
    await sb(supabase.table('users').update({'lifetime_fees_avoided': current_savings + fees_avoided}).eq('id', current_user['id']))
...
async def parse_email(email_data: EmailParseRequest, current_user: dict = Depends(get_current_user)):
    ...
    shipment_doc = {
        "id": shipment_id, "user_id": current_user["id"], "container_number": container,
        "vessel_name": parsed.get("vessel") or "Unknown", "carrier": parsed.get("carrier") or "Unknown",
        "arrival_date": parsed.get("arrival_date") or datetime.now(timezone.utc).isoformat(),
        "last_free_day": lfd, "notes": f"Parsed from email: {email_data.subject}",
        "status": status, "hours_remaining": round(hours, 1),
        "created_at": datetime.now(timezone.utc).isoformat(), "source": "email"
    }
    await sb(supabase.table('shipments').insert(shipment_doc))
...
async def parse_and_process_shipment(content: str, content_type: str, customer: dict, source: str, filename: str = None):
    ...
    try:
        ...
        if len(lfd) == 10:
            lfd = f"{lfd}T23:59:59Z"
        status, hours = calculate_shipment_status(lfd)
        existing = await sb(supabase.table('shipments').select('*').eq('user_id', customer['id']).eq('container_number', container_id).maybe_single())
        ...
        if is_update:
            ...
            await sb(supabase.table('shipments').update({
                "carrier": carrier, "vessel_name": vessel, "arrival_date": arrival_date,
                "last_free_day": lfd, "status": status, "hours_remaining": round(hours, 1),
            ...
            await sb(supabase.table('shipments').insert({
                "id": shipment_id, "user_id": customer["id"], "container_number": container_id,
                "carrier": carrier, "vessel_name": vessel, "arrival_date": arrival_date,
                "last_free_day": lfd, "status": status, "hours_remaining": round(hours, 1),
        ...
        if customer_phone:
            ...
            try:
                ...
                await sb(supabase.table('sms_logs').insert({
                    "id": str(uuid.uuid4()), "user_id": customer["id"], "shipment_id": shipment_id,
                    "container_number": container_id, "message": msg,
                    "notification_type": "delay_alert" if is_update else "initial_tracking",
                    "sent_at": datetime.now(timezone.utc).isoformat(), "status": "sent_real",
                    "twilio_sid": sms_result.get("sid")
                }))
            except Exception as e:
                logger.error(f"SMS send error: {e}")
        return {"status": "success", "action": "updated" if is_update else "created", "container_id": container_id, "lfd": lfd[:10], "carrier": carrier, "filename": filename}
    except Exception as e:
...
async def get_notification_settings(current_user: dict = Depends(get_current_user)):
    return current_user.get("notification_settings") or {"notify_48h": True, "notify_24h": True, "notify_12h": True, "notify_6h": True}
...
async def update_notification_settings(settings: NotificationSettingsUpdate, current_user: dict = Depends(get_current_user)):
    current = current_user.get("notification_settings") or {"notify_48h": True, "notify_24h": True, "notify_12h": True, "notify_6h": True}
    updated = dict(current)
    if settings.notify_48h is not None: updated["notify_48h"] = settings.notify_48h
    if settings.notify_24h is not None: updated["notify_24h"] = settings.notify_24h
    if settings.notify_12h is not None: updated["notify_12h"] = settings.notify_12h
    if settings.notify_6h is not None: updated["notify_6h"] = settings.notify_6h
...
async def get_notification_logs(current_user: dict = Depends(get_current_user)):
    result = await sb(supabase.table('sms_logs').select('*').eq('user_id', current_user['id']).order('sent_at', desc=True).limit(100))
...
async def check_and_send_notifications(current_user: dict = Depends(get_current_user)):
    ...
    for s in shipments:
        ...
        if 5.5 <= hours <= 6.5 and settings.get("notify_6h", True): notification_type = "6h"
        elif 11.5 <= hours <= 12.5 and settings.get("notify_12h", True): notification_type = "12h"
        elif 23.5 <= hours <= 24.5 and settings.get("notify_24h", True): notification_type = "24h"
        elif 47.5 <= hours <= 48.5 and settings.get("notify_48h", True): notification_type = "48h"
        if notification_type:
            existing = await sb(supabase.table('sms_logs').select('id').eq('shipment_id', s['id']).eq('notification_type', notification_type).maybe_single())
            if not existing.data:
                log = {"id": str(uuid.uuid4()), "user_id": current_user["id"], "shipment_id": s["id"], "container_number": s["container_number"], "message": f"LFD Alert: Container {s['container_number']} expires in {notification_type}.", "notification_type": notification_type, "sent_at": datetime.now(timezone.utc).isoformat(), "status": "sent_mock"}
                await sb(supabase.table('sms_logs').insert(log))
                notifications_sent.append(log)
    return {"message": f"Checked {len(shipments)} shipments, sent {len(notifications_sent)} notifications", "notifications": notifications_sent}
...
async def seed_demo_data(current_user: dict = Depends(get_current_user)):
    ...
    for data in demo_shipments:
        existing = await sb(supabase.table('shipments').select('id').eq('user_id', current_user['id']).eq('container_number', data["container_number"]).maybe_single())
        if not existing.data:
            shipment_id = str(uuid.uuid4())
            status, hours = calculate_shipment_status(data["last_free_day"])
            doc = {"id": shipment_id, "user_id": current_user["id"], **data, "status": status, "hours_remaining": round(hours, 1), "created_at": datetime.now(timezone.utc).isoformat(), "source": "demo"}
...
async def test_email_parse_and_sms(request: TestEmailSMSRequest, current_user: dict = Depends(get_current_user)):
    if not GEMINI_API_KEY:
        raise HTTPException(status_code=500, detail="Gemini API key not configured")
    prompt = f"""You are a logistics document parser. Extract shipment information and return ONLY valid JSON with these fields:
- container_number, vessel_name, arrival_date (ISO format), last_free_day (ISO format), bill_of_lading
Parse dates carefully. Return ONLY JSON.\n\n{request.email_content}"""
    try:
        raw = await call_gemini_text(prompt)
        parsed = clean_json_response(raw)
    except Exception as e:
    ...
    status, hours = calculate_shipment_status(lfd)
    existing = await sb(supabase.table('shipments').select('*').eq('user_id', current_user['id']).eq('container_number', container).maybe_single())
    existing_shipment = existing.data
    is_update = bool(existing_shipment)
    old_lfd = None
    if is_update:
        ...
        shipment_id = existing_shipment["id"]
        await sb(supabase.table('shipments').update({"vessel_name": vessel, "arrival_date": arrival, "last_free_day": lfd, "status": status, "hours_remaining": round(hours, 1), "updated_at": datetime.now(timezone.utc).isoformat()}).eq('id', shipment_id))
        shipment_doc = {"id": shipment_id, "user_id": current_user["id"], "container_number": container, "vessel_name": vessel, "last_free_day": lfd, "status": status, "hours_remaining": round(hours, 1), "source": "test_email"}
    else:
        shipment_id = str(uuid.uuid4())
        shipment_doc = {"id": shipment_id, "user_id": current_user["id"], "container_number": container, "vessel_name": vessel, "arrival_date": arrival, "last_free_day": lfd, "notes": "Created via Test Email Parse", "status": status, "hours_remaining": round(hours, 1), "created_at": datetime.now(timezone.utc).isoformat(), "source": "test_email"}
        await sb(supabase.table('shipments').insert(shipment_doc))
    if is_update:
        sms_message = f"LFD Clock: UPDATED! Container: {container}\nOld LFD: {old_lfd}\nNEW LFD: {lfd[:10]}\nStatus: {status.upper()}\nTime: {round(hours, 1)}h"
    else:
        sms_message = f"LFD Clock: New shipment!\nContainer: {container}\nVessel: {vessel}\nLFD: {lfd[:10]}\nStatus: {status.upper()}\nTime: {round(hours, 1)}h"
    sms_result = send_real_sms(request.phone_number, sms_message)
    await sb(supabase.table('sms_logs').insert({"id": str(uuid.uuid4()), "user_id": current_user["id"], "shipment_id": shipment_id, "container_number": container, "message": sms_message, "notification_type": "update" if is_update else "new", "sent_at": datetime.now(timezone.utc).isoformat(), "status": "sent_real", "twilio_sid": sms_result.get("sid")}))
    return {"success": True, "action": "updated" if is_update else "created", "parsed_data": parsed, "shipment": shipment_doc, "sms": {"sent_to": sms_result.get("to"), "twilio_sid": sms_result.get("sid")}}
...
async def process_pending_reminders():
    now = datetime.now(timezone.utc)
    result = await sb(supabase.table('reminders').select('*').eq('status', 'pending').lte('schedule_time', now.isoformat()).limit(1000))
    ...
    for reminder in pending:
        ...
        try:
            ...
            if not user_settings.get(f"notify_{reminder['reminder_type']}", True):
            ...
            msg = f"Alert: Container {container} expires in {reminder.get('hours_before', '?')} hours. Avoid storage fees!"
            sms_result = send_real_sms(phone, msg)
            await sb(supabase.table('reminders').update({"status": "sent", "processed_at": now.isoformat(), "twilio_sid": sms_result.get("sid")}).eq('id', reminder['id']))
            await sb(supabase.table('sms_logs').insert({"id": str(uuid.uuid4()), "user_id": reminder["user_id"], "shipment_id": reminder["shipment_id"], "container_number": container, "message": msg, "notification_type": f"reminder_{reminder['reminder_type']}", "sent_at": now.isoformat(), "status": "sent_real", "twilio_sid": sms_result.get("sid")}))
            results["sent"] += 1
            results["details"].append({"container": container, "type": reminder["reminder_type"], "status": "sent"})
        except Exception as e:
            logger.error(f"Reminder error {reminder.get('id')}: {e}")
...
async def get_user_reminders(current_user: dict = Depends(get_current_user)):
    result = await sb(supabase.table('reminders').select('*').eq('user_id', current_user['id']).order('schedule_time', desc=False))
    return result.data or []
...
async def direct_upload_pdf(file: UploadFile = File(...), current_user: dict = Depends(get_current_user)):
    ...
    with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as tmp:
        ...
        tmp_path = tmp.name
    try:
        return await parse_and_process_shipment(content=tmp_path, content_type="pdf", customer=current_user, source="direct_upload", filename=file.filename)
    finally:
        if os.path.exists(tmp_path):
            os.unlink(tmp_path)
...
async def share_with_trucker(shipment_id: str, request: TruckerShareRequest, current_user: dict = Depends(get_current_user)):
    ...
    msg = f"Hi {request.trucker_name or 'Driver'}, pickup needed:\n\nContainer: {container}\nVessel: {shipment.get('vessel_name', '')}\nLFD: {lfd}\nTime left: {round(hours)}h\n\nFrom: {current_user.get('company_name', 'LFD Clock')}"
    sms_result = send_real_sms(request.trucker_phone, msg)
    await sb(supabase.table('sms_logs').insert({"id": str(uuid.uuid4()), "user_id": current_user["id"], "shipment_id": shipment_id, "container_number": container, "message": msg, "notification_type": "trucker_share", "sent_at": datetime.now(timezone.utc).isoformat(), "status": "sent_real", "twilio_sid": sms_result.get("sid"), "trucker_phone": request.trucker_phone}))
...
async def get_carrier_portal_link(shipment_id: str, current_user: dict = Depends(get_current_user)):
    ...
    return {"carrier": carrier, "portal_url": get_carrier_portal(carrier), "container_number": shipment.get("container_number")}
...
async def reminder_scheduler():
    while True:
        try:
            ...
            now = datetime.now(timezone.utc)
            result = await sb(supabase.table('reminders').select('*').eq('status', 'pending').lte('schedule_time', now.isoformat()).limit(100))
            pending = result.data or []
            for reminder in pending:
                try:
                    user_result = await sb(supabase.table('users').select('*').eq('id', reminder['user_id']).maybe_single())
